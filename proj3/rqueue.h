...
private:
  ...
  void recursiveCopy(Node* srcNode, Node*& copyNode);
  Node* merge(Node* heapOne, Node* heapTwo);
  int updateSize(Node* node);
  void printStudentPreorder(Node* node) const;
  void skewHeapToArray(Node* node, Student studs[]);
