...
public:
    ...
    void updateSize(DNode* node, DNode* prevRoot, int prevRootDiscriminator);
...
private:
    ...
    void deleteTree(DNode* node,bool deleteNode);
    void recursiveCopy(DNode* srcNode, DNode*& copyNode);
    bool recursiveInsert(DNode*& localRoot, Account acct, bool isCreated);
    DNode* recursiveSearch(DNode* node, int disc);
    DNode* recursiveRemove(DNode* node, int disc);
    void storeDNodes(DNode* node);
    void traversal(DNode* node, int* index, DNode* accountArray, int arraySize);
    DNode* rebuildTree(int begin, int end, DNode* accountArray, DNode*& node, DNode*& prevRoot, int prevRootDiscriminator);
