...
int main(){
   ...
   cout<<"INTITATING REMOVE FUNCTION"<<endl;
   for (int i = 0;i<14;i++){
   ...
   //Testing to input in already inputted object
   aTable.insert(Person("John", 41953));
   HashTable bTable(101,hashCode);
   //Testing insertion
   int arr[69];
   for(int i = 0; i < 69;i++){
      int num = vaccineSerial();
      arr[i] = num;
      bTable.insert(Person("John", num));
   }
   bTable.dump();
   //Testing remove
   for(int i = 0; i < 69;i++){
      bTable.remove(Person("John", arr[i]));
   }
   
   cout<<"TESTING TO REMOVE FROM EMPTY HASHTABLE"<<endl;
   bTable.remove(Person("John", arr[0]));
   return 0;
}
