...
void Tester::measureCopyTime(int numTrials,int N){
    ...
    for(int k = 0; k < numTrials-1;k++){
        ...
        stop = clock();
        T = stop - start;
        cout<<"Copying "<<copy.m_size * copy.m_size<<" members took "<< T << " clock ticks ("<<T/CLOCKS_PER_SEC << " seconds)!" << endl;
        N = N * a;
...
bool Tester::testAssignmentOperator(const Puzzle& puzzle){
    ...
    Puzzle temp = puzzle;
    temp = temp;
    //Same table size but different table pointers
    if((temp.m_size == puzzle.m_size) && (puzzle.m_table != temp.m_table) ){
        for (int i=0;i<puzzle.m_size;i++){
            for (int j=0;j<puzzle.m_size;j++){
                if ((puzzle.m_table[i][j] != temp.m_table[i][j]) || //checks values
                    (puzzle.m_table[i] == temp.m_table[i])){       //checks the ith pointer
                    return false;
                }
            }
        }
...
bool Tester::testCopyConstructor(const Puzzle& puzzle){
    ...
    //the case that sizes are the same and the table pointers are not the same
        ...
        for (int i=0;i<puzzle.m_size;i++){
            for (int j=0;j<puzzle.m_size;j++){
                if ((puzzle.m_table[i][j] != copy.m_table[i][j]) || (puzzle.m_table[i] == copy.m_table[i])){       
                    return false;
                }
            }
        }
